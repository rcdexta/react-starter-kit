...
This project aims to be an opinionated starter kit for building UI applications using React.
...
```
git clone git@github.com:serviceslabs/react-starter.git --starter react-starter-kit your_project_name
cd your-project-name
yarn install
```

After installing all the dependencies and locking them as `yarn.lock` you should be ready to start you server
...
This will run all the test files along with coverage enabled. Good to run in it on CI to certify a build. The coverage threshold has been set to `50%` for now
...
You can always pull the changes from the `react-starter-kit` upstream

```
git pull react-starter-kit master
```

Any updates changes in the starter kit repo should be available in your project now

Remember to run `yarn install` to update any new dependencies
...
The generated project basic dependencies to get you started. It also includes a set of scripts used by Create React App as a development dependency. You may install other dependencies (for example, React-Material) with `yarn`:
...
You can add your own custom CSS stylesheets and use them like below:
...
But, we recommend that you use [styled-components](https://github.com/styled-components/styled-components) that uses css-in-js for styling the components. A short example below:
